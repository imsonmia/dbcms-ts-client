...
const verify = async (
  ...
  if (!response.success) {
    return { user: undefined, response: VerificationStatus.serverSideError };
  }
  ...
  handleVerifyClick = (
    ...
    verify(context, hashPasswd, usernameInputVal)
      .then((value) => {
        switch (value.response) {
          ...
          case VerificationStatus.invalidUsername:
            return {
              msg: Keywords.loginAttemptInvalidUsernm,
              verified: false,
              user: value.user,
            };
      ...
      .then((value) => {
        ...
        (
          document.getElementById("p-query-resolve") as HTMLParagraphElement
  ...
  render() {
    return (
      <div className="login">
        ...
        <input type="password" required={true} id="input-password"></input>
        <br></br>
        <button
          onClick={(ev) => {
